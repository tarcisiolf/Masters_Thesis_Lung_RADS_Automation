...
if __name__ == "__main__":
    train_data = dp.load_json(r"1_bilstmcrf_pytorch\train_test_70_30\data\train_data.json")
    test_data = dp.load_json(r"1_bilstmcrf_pytorch\train_test_70_30\data\test_data.json")
    unique_tags = dp.load_json(r"1_bilstmcrf_pytorch\train_test_70_30\data\unique_tags.json")
    word2index = dp.load_json(r"1_bilstmcrf_pytorch\train_test_70_30\data\word2index.json")
    index2word = {int(k): v for k, v in dp.load_json(r"1_bilstmcrf_pytorch\train_test_70_30\data\index2word.json").items()}
    tag2index = dp.load_json(r"1_bilstmcrf_pytorch\train_test_70_30\data\tag2index.json")
    index2tag = {int(k): v for k, v in dp.load_json(r"1_bilstmcrf_pytorch\train_test_70_30\data\index2word.json").items()}
    ...
    for batch_size, embedding_dim, hidden_dim, lstm_dropout, learning_rate in itertools.product(
        ...
        for i in range(5):
            ...
            folder_name = r"1_bilstmcrf_pytorch\train_test_70_30\models"
            model_file_name = folder_name+fr"\bilstm_crf_batch_size={batch_size}_embedding_dim={embedding_dim}_hidden_dim={hidden_dim}_lstm_dropout={lstm_dropout}_learning_rate={learning_rate}.pth"
            ...
            output_file_name = "1_bilstmcrf_pytorch\train_test_70_30\metrics\"+"f"evaluation_report_batch_size={batch_size}_embedding_dim={embedding_dim}_hidden_dim={hidden_dim}_lstm_dropout={lstm_dropout}_learning_rate={learning_rate}_run_{i}.json"
